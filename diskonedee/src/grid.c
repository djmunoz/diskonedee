 ...
 for (int j=1; j < M-1; j++)
    {
      ...
      /*
      delta_inv = 1.0 / (G->vals[1].side_val - G->vals[0].side_val);
      ...
      */
      ...
      /*
      G->vals[j].A_coeff_val = params.AlphaCoefficient;
      G->vals[j].B_coeff_val = -2 * params.AlphaCoefficient;
      G->vals[j].C_coeff_val = params.AlphaCoefficient;
      */
    }
